...
/** Whole-dollar amount, e.g. "$2,304". */
const usd = (n: number) => `$${Math.round(n).toLocaleString("en-US")}`;

// Archive comparison scenario: 100 TB stored, one 10 TB restore per year. AWS eu-west-1 list rates, Sept 2026.
// AWS S3 Standard: 51,200 GB x $0.023 + 51,200 GB x $0.022 = $2,304/month; 10 TB restore = 10,240 GB x $0.09 = $922.
// AWS Glacier Instant: storage 102,400 x $0.004 = $410; 10 TB restore = 10,240 x ($0.03+$0.09) = $1,229.
// AWS Glacier Deep Archive: storage 102,400 x $0.00099 = $101; 10 TB restore = 10,240 x ($0.02+$0.09) = $1,126.
// Wasabi: 100 TB x $7.99 = $799/month, $0 retrieval. Fil One: 100 TB x PRICE_PER_TB, $0 retrieval.
const config: LandingPageConfig = {
  ...
  hero: {
    ...
    description: `Flat ${PRICE_PER_TB_SHORT}. No egress and no retrieval tax, so testing a restore doesn't cost you anything extra.`,
    ctas: [
  ...
  comparison: {
    ...
    rows: [
      ...
      { provider: "AWS Glacier Instant", values: { storage: "$410", rate: "$0.03 + $0.09/GB", restore: "$1,229" } },
      ...
      { provider: "Fil One", isFilOne: true, values: { storage: usd(100 * PRICE_PER_TB), rate: "$0", restore: "$0" } },
    ],
    footnote:
      `AWS S3 Standard and Glacier list rates for eu-west-1, September 2026. S3 Standard storage: tiered $0.023/$0.022/GB; Glacier Instant $0.004/GB; Glacier Deep Archive $0.00099/GB. Retrieval: Glacier Instant $0.03/GB, Deep Archive $0.02/GB, each plus $0.09/GB egress. Restore cost computed: 10,240 GB × stated rates. Wasabi $7.99/TB. Fil One ${PRICE_DISPLAY}/TB, $0 retrieval, $0 egress.`,
  },
    ...
    items: [
      {
        ...
        desc: "Objects are readable the instant you request them, with no rehydration delay like Glacier's retrieval tiers. A restore starts immediately.",
      },
      {
        icon: ArrowsOut,
