    ...
    <div className="min-h-screen bg-white">
      ...
      <main id="main-content" className="pt-[58px] md:pt-[94px]">
        {loading ? (
          ...
          <article>
            <header className="px-5 pb-10 pt-12 md:px-8 md:pb-16 md:pt-20">
              <div className="mx-auto max-w-[820px]">
                <a href="/blog" className="inline-flex items-center gap-2 text-sm text-[#52525B] no-underline transition-colors hover:text-[#09090B]"><ArrowLeft size={15} /> All articles</a>
                <p className="mt-10 font-mono text-[11px] uppercase tracking-[0.14em] text-[#71717A]">{formatDate(post.publishedAt)} · {post.author}</p>
                <h1 className="mt-5 font-['Aspekta'] text-[38px] font-medium leading-[1.08] tracking-[-0.035em] text-[#09090B] md:text-[60px]">{post.title}</h1>
                <p className="mt-6 max-w-2xl text-lg leading-8 text-[#52525B] md:text-xl">{post.excerpt}</p>
              </div>
            </header>

            <div className="mx-auto aspect-[16/9] max-w-[1120px] overflow-hidden md:rounded-2xl">
              <BlogCover post={post} priority />
            </div>

            <div className="px-5 py-12 md:px-8 md:py-20">
              <div className="blog-content mx-auto max-w-[700px]" dangerouslySetInnerHTML={{ __html: safeContent }} />
            </div>
          </article>
        )}
      </main>
