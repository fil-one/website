...
/** AWS EFS Standard, $0.30/GB x 1,024 GB. */
const EFS_PER_TB = 307.2;
const FIL_ONE_10TB = 10 * PRICE_PER_TB;
/** How many times cheaper per TB than EFS, rounded down so copy never overstates it. */
...
// AWS S3 Standard eu-west-1: $0.023/GB storage ($235.52) + 20 runs x 10,240 GB x $0.09/GB egress
...
const config: LandingPageConfig = {
    ...
    items: [
      {
        ...
        body: "AWS EFS costs $0.30/GB ($307/TB) because it is optimized for latency, not cost. A 10 TB training corpus costs $3,072/month. Teams pay that to avoid the alternative: slow, metered S3.",
      },
      {
  ...
  comparison: {
    ...
    rows: [
      ...
      { provider: "Fil One", isFilOne: true, values: { rate: PRICE_PER_TB_SHORT, egress: "$0", total: usd(FIL_ONE_10TB) } },
    ],
    footnote:
      `AWS EFS gp bursting us-east-1: $0.30/GB. AWS S3 Standard eu-west-1: $0.023/GB storage + $0.09/GB egress per read. Computed: EFS 10,240 GB × $0.30 = $3,072; S3 storage $235.52 + 20 runs × 10,240 GB × $0.09 = $18,432 egress. Fil One: 10 TB × ${PRICE_DISPLAY} = $${FIL_ONE_10TB.toFixed(2)}, egress $0. Q2 2026 public rate cards.`,
  },
  ...
  features: {
    ...
    items: [
      {
        icon: ChartLine,
        title: `${EFS_MULTIPLE}× cheaper than EFS`,
        desc: `AWS EFS costs $0.30/GB ($307/TB). Fil One costs ${PRICE_PER_TB_SHORT}. At 10 TB of training data, that is $3,072/month vs ${usd(FIL_ONE_10TB)}, before a single training run reads a byte.`,
      },
      {
        icon: ArrowsOut,
