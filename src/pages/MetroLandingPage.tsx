...
import { COMPETITORS, PRICE_PER_TB_SHORT, monthlyCost } from "@/lib/pricing";
...
const FIL_ONE = byName("Fil One");

/** Storage, egress, and total cells for 10 TB stored and 10 TB read per month. */
const cells = (c: (typeof COMPETITORS)[number]) => {
  const storage = monthlyCost(c, 10, 0);
  const total = monthlyCost(c, 10, 10);
  return { storage: usd(storage), egress: usd(total - storage), total: usd(total) };
};
...
// AWS S3 Standard eu-west-1: 10,240 GB x $0.023 storage + 10,240 GB x $0.09 egress
//   = $235.52 + $921.60 = $1,157.12.
// Wasabi, Backblaze B2 (10 TB read is inside its 3x free egress), Cloudflare R2, and
// Fil One are computed from COMPETITORS via monthlyCost() above.
const config: LandingPageConfig = {
  ...
  hero: {
    badge: "For SaaS and creative teams",
    titleMaxWidth: 720,
    ...
    items: [
      {
        ...
        body: "AWS S3 Standard and other hyperscaler hot tiers charge premium rates for storage with no archive delay and no rehydration wait. Teams pay hyperscaler prices just to avoid the archive-tier trade-off.",
      },
      {
  ...
  comparison: {
    ...
    sub: "Same workload, five providers. Storage is a small line; egress is the bill on hyperscalers. On Fil One, egress does not exist as a line item.",
    subMaxWidth: 620,
    ...
    rows: [
      ...
      { provider: "Cloudflare R2", values: cells(R2) },
      { provider: "Wasabi", values: cells(WASABI) },
      { provider: "Backblaze B2", values: cells(B2) },
      { provider: "Fil One", isFilOne: true, values: cells(FIL_ONE) },
    ],
    footnote:
      `AWS S3 Standard eu-west-1, public rate card Q2 2026: 10,240 GB × $0.023 storage + 10,240 GB × $0.09 egress. Cloudflare R2 $${R2.storagePricePerTB}/TB, $0 egress. Wasabi $${WASABI.storagePricePerTB}/TB, $0 egress. Backblaze B2 $${B2.storagePricePerTB}/TB, egress free up to ${B2.freeEgressMultiplier}x stored. Fil One ${PRICE_PER_TB_SHORT}, $0 egress.`,
  },
