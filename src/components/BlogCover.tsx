...
/** Decorative gradient covers for posts with no featured image (cyan = aqua-300 mid stop). */
const coverClasses: Record<CoverStyle, string> = {
...
const BlogCover = ({ post, priority = false }: { post: BlogPost; priority?: boolean }) => {
  ...
  return (
    <div className={`relative h-full w-full overflow-hidden bg-gradient-to-br ${palette}`} aria-hidden="true">
      ...
      <div className="absolute bottom-5 right-5 font-mono text-[10px] uppercase tracking-[0.18em] text-black/55">Fil One / Blog</div>
    </div>
  );
};
