 ...
 *
 * There are deliberately no per-currency or per-locale variants below. The
 * Stripe plan is denominated in USD only, so every page quotes the USD rate in
 * this one format, the Spanish pages included. If native EUR billing ships, the
 * euro price becomes its own constant at that point.
 */
